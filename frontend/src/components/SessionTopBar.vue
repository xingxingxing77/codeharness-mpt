<template>
  <div class="topbar">
    ...
    <span class="cost" :title="costTip">
      ¥{{ (cost.total_cost ?? 0).toFixed(3) }}
    </span>
    <button class="icon-btn" title="审查代码变更" @click="openReview">
      <Icon name="sliders" :size="18" />
    </button>
    <button class="icon-btn" title="终端面板" @click="ui.terminalOpen = !ui.terminalOpen">
      <Icon name="monitor" :size="18" />
    </button>
    <button class="icon-btn" title="工具面板" @click="ui.toggleRight()">
      <Icon name="panel-right" :size="18" />
    </button>
    <div v-if="menuMore" class="overlay" @click="closeMenus" />
  </div>
</template>
