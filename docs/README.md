...
| `施工5-前端重构-F1-F7.md` | 界面 1:1 复刻 deepseek-harness，只接本项目后端；令牌层/布局契约/block 映射/钩子契约/验证手段 | 改前端前先读「明确不做的东西」 |
| `adr/` | **技术决策记录**：不可逆的口径决定（背景/决策/后果/重开条件），含被拒选项与「取证推翻原方案」那类 | 要改口径、或想问「当初为什么不这么做」时先查这里；执行状态仍以 `plan/PLAN.md` 为准 |
| `前端/接口契约.md` | 21 条 FastAPI 路由总表 + 请求体 422 判据 + Session/Span/Event 形状与单位口径 + SSE 游标铁律 + 改接口要同步改哪几处 | 前端要加控件前先查第 7 节：没有路由就没有控件 |
...
### 2026-09-16 下午 · S6 开工（批1/2a/2c 类图/2b/批3 五片，提交 `89283e9`…`74cb8e8`）
...
- **接线批 B2c ✅（同日，台账 #6 收口）**：RunCode 按复盘 `Send To` 分诊通电（parse_recipient 逐字件从无人调用变成路由判据）：Engineer→具名投 + instruct=CodingContext 形态，其余→QA `<self>` 自环 + instruct=RunCodeContext 形态；**DebugError 修复产物进 tests/**（源 qa_engineer.py:155，此前无条件写 SRC/ 污染源码）；QA 测试修复自环上 `debug_rounds` 闸（源 test_round 语义，此前只有 QA↔Engineer 边有数）。复查还揪出旧"回流键名对齐"是**假回路**：RunCodeOutput 的 ok/output_filename 进 CodingContext 会被 extra=forbid 当场拒——B2c 把三跳回流全改成"恰好消费方 schema"形态，s6 t23 每跳都验形态兼容。
- **接线批 B2b ✅（同日，台账 #4/#5 收口）**：增量重写链通电——`WriteCodePlanAndChange` 从 20 行壳加厚为源六段上下文（REFINED_TEMPLATE/CODE_PLAN_AND_CHANGE_CONTEXT 构建脚本 AST 摘取、t11 值比对入册），`Agent` 新增**按因装配** `plans/default_plan`（源 engineer._new_code_actions 浓缩，构造即校验两张表互洽），FIX_BUG → 计划 → WriteCode 走 REFINED（use_inc 旧码置顶含源 main.py 例外）→ 评审 → 摘要，s6 t22 全链行为钉。顺带合并 `BUGFIX_FILENAME`/`DocName.BUGFIX` 同物两值死孪生——接线瞬间抓出的又一处"静默查不到文件"。
