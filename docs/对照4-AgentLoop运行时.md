...
3. **ToT / solver / search_space / action_graph 四件属漏判**：既不在 `判定-复制与重构清单.md` 的「复/改」表也不在「弃」表（清单 `:119` 只列了 `planner/thinking_command/base/task_type/experience_retriever`），`docs/README.md` 与四份施工文档 grep `tot|ToT|思维树` 零命中。现状是未实现，但**没有"决定不做"的记录**——这与 §零 预算那条明确判弃形成对照，需要补一条判定。
...
| checkpoint 断点续跑 | `team.py:59-81` | `checkpoint.py:37-91` | 现代化替换 | 但 `runner._run` 未传 `interrupt_before`（§五-B） |
...
| 按因装配动作序 | `engineer.py:_new_code_actions` | `agent.py:31-41` + `team.py:138-141` | 已复刻 | `plans`/`default_plan` 构造即 assert 校验（`:38,41`） |
| 策略可插拔 sop/react/role_zero/plan_and_act | `role.py:82-89,261-282` | `registry.py:231-249`; `team.py:74-81` | 部分实现 | 三档可换且非法值 raise；`plan_and_act` 是**另一个类**，不经 `Agent`（§五-F） |
...
| ToT / 树搜索 | `strategy/tot.py:1-277`、`solver.py:44-77`、`search_space.py`、`strategy/base.py`(109) | 无 | **未实现（且漏判）** | §结论-3 |
...
- **F. `profile["strategy"]` 是只写字段**：`agent.py:46`、`role_zero.py:52`、`registry.py:247` 三处写，全仓**无读者**（含 `server/`、前端契约）。换装的真实入口是 `build_role(strategy=)`（`registry.py:231-249`，对 RoleZero 族显式 raise `:244`，对未知值 raise `:249`——这两处是硬的）。另 `team.react_assembly:74-81` 只改 `r.react_mode` 不改 `profile["strategy"]`，两者会漂移；因为无读者，目前无害。
...
零断言：经典线 interrupt（因为无实现）；`recursion_limit` 触发后的状态归因；`ToT/solver`（无实现）；`debug_rounds` 两处硬闸的上限行为只在 QA 自环一例里间接见过，没有独立断言。
...
3. **补记 ToT 系四件的判定**（复/改/重/弃任一处），别留漏判。
