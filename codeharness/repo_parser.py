...
from codeharness.const import AGGREGATION, COMPOSITION, GENERALIZATION
from codeharness.logs import logger
from codeharness.utils.common import any_to_str, aread, remove_white_spaces
...
class RepoParser(BaseModel):
    ...
    def _create_path_mapping(path: str | Path) -> Dict[str, str]:
        ...
        """
        # Windows 校准：键必须与 pyreverse 的点分命名空间可匹配——反斜杠先折叠再换点，
        # 值统一存正斜杠形态（Path 两种都吃，且 update_graph_db_with_class_views 按 ":" 切前缀比后缀名）
        def _key(p) -> str:
            return str(p).replace("\\", "/").replace("/", ".")
        path_s = str(path).replace("\\", "/")
        mappings = {_key(path): path_s}
        files = []
        try:
        ...
        for f in files:
            mappings[_key(Path(f).with_suffix(""))] = str(f).replace("\\", "/")
    ...
    def _repair_namespaces(
        ...
        if not class_views:
            return [], [], ""
        c = class_views[0]
        # Windows 校准（判 `改` 的核心一处）：源假设 "/" 分隔的 POSIX 绝对路径。两件事：
        # ① 反斜杠折叠成斜杠；② `C:` 盘符补一个前导 "/" 使其与 "/tmp/..." 同构——
...
class CodeChunk(BaseModel):
    """= 源 repo_parser.CodeBlockInfo 的切分等价物（type/name/content）"""
...
def _tree_sitter_chunks(code: str) -> list:
...
def _regex_chunks(code: str) -> list:
    out = []
...
def parse_file(file_path: str, root: str = "") -> list:
...
def parse_repository(repo_dir: str) -> list:
