...
@Author  : alexanderwu
@File    : memory.py（复制自 metagpt/memory/memory.py，按 docs/04 §4.1 改 3 处 import）
...
from codeharness.const import IGNORED_MESSAGE_ID
from codeharness.schema import Message
def any_to_str(v) -> str:
    """内联自 metagpt/utils/common.py：类/对象 → tag 字符串"""
    return v if isinstance(v, str) else v.__name__ if isinstance(v, type) else str(v)


def any_to_str_set(v) -> set:
    return {any_to_str(i) for i in v}
