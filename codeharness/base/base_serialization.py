#!/usr/bin/env python
# -*- coding: utf-8 -*-
"""多态子类序列化/反序列化 Mixin。来源：metagpt/base/base_serialization.py 逐字复制（零改写）。

这个文件**不判 `重`**，与判定表 R4 无关：R4 取代的是"存档到哪儿"
（`serialize`/`deserialize`/`get_serialization_path` 那半边，可被 checkpointer 替代），
而这里解决的是"**多态还原**"——一个与存储后端正交的问题，checkpointer 同样需要它：
字段声明成基类、值实际是子类时，普通 pydantic dump/validate 会把子类字段丢掉。

源项目零 metagpt 依赖（只 import typing + pydantic），所以整文件原样复制。
"""
...
class BaseSerialization(BaseModel, extra="forbid"):
    ...
    # 序列化时：自动注入类名
    # __serialize_with_class_type__ 方法在序列化时自动追加 __module_class_name 字段，
    # 记录这个对象的真实子类是谁：
    ...
    # 反序列化时：根据类名还原真实类型
    # __convert_to_real_type__ 方法在反序列化时读取 __module_class_name，
    # 从 __subclasses_map__ 中查找对应的子类，然后用正确的子类来构造对象：
    ...
    # 自动注册子类
    # __init_subclass__ 让每个继承 BaseSerialization 的子类自动注册到全局映射表中：
